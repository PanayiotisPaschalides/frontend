...
import CreateAdd from "./components/AdCreation/AdCreation"
...
class App extends Component {
  ...
  componentDidMount(){
    ...
    if(User !== undefined && ID !== undefined )
    {
        ...
        this.setState({logUser: User, UserID: ID, Page:'Home', LoggedIn: true})
...
onLoginSubmit(event){
    const self = this;
    axios.get(config.Get_Login_Details, {
        params: {
    ...
    .then(function (response) {
        if(response.data.status)
        {
          ...
          cookies.set('User', self.state.Username, { path: '/' });
...
onSignupSubmit(){
    ...
    else
    {
        ...
        axios.post(config.Post_Register,user)
        .then((response)=>{
            if(response.data.status){
                self.SignupClose();
                self.LoginOpen();
...
onManageRecipesClick(event){
    const self = this;
    axios.get(config.Get_Recipes_Managment, {
        params:{
...
publish(ID){
    ...
    axios.post(config.Post_Update_Recipe_Status,recipe)
    .then((response)=>{
        if(response.data.status){
...
OpenRecipe(ID){
        ...
        axios.post(config.Post_Update_Recipe_Views,recipe)
        .then((response)=>{
            if(response.data.status){
                self.LoadRecipe(ID);
            }
...
CheckForNewMessages(){
    axios.get(config.Get_New_Messages, {
        params:{
    ...
    if(this.state.Page === 'Home'){
        PageBody = (
            ...
            <div key={Advert.id} className="Album" >
              <img src={config.api_host + '/img/'+ Advert.image} 
              className="Album-Img" alt="Album" onClick={() => this.OpenRecipe(Advert.id)}/>
